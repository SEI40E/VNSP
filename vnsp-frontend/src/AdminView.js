...
import {addVehicle, baseUrl, getVehicles} from "./VehicleForm";
...
const AdminView = () => {
    ...
    const filteredvehicles = vehicleVariable.filter(vehicle => {
        const vehicleStart = vehicle.Start.toLowerCase();
        const vehicleDestination=vehicle.Destination.toLowerCase();
        const vehicleType=vehicle.Type.toLowerCase();
        const searchTerm = searchTermVariable.toLowerCase();
